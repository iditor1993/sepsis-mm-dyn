...
| `features/vitals_hourly_v2.parquet`、`labs_hourly_v2.parquet`、`sofa_hourly_v2.parquet`、`nee_stream_v2.parquet` | 小时级长表（含 bin 聚合字段、`source_time_type`、双轨） | P2 张量化 |
| `features/vitals_realtime_strict_v2.parquet`、`vitals_charttime_retro_v2.parquet` | 生命体征双轨 | 双轨敏感性 |
...
- 同 bin 多条：已由提取层聚合（中位数），本层仅校验 `aggregation_method = 'median'` 与 `max_available_time ≤ t_landmark`（strict 轨）；不再二次聚合。
...
- SOFA：按 `sofa_track` 选择 `strict_24h`（主）或 `carryforward`（敏感性）轨的逐组分与 `sofa_total_complete`；`partial` 总分不进主输入（提取方案 §5.4）；CV 亚组字段固定 `sofa_realtime_strict_24h_cv`（首个有效 landmark、患者级）。
- NEE 流：按 5min 网格重采样至 1h 网格（bin 内最大值为该 bin 代表值，与「24h 最差」语义一致），通道名 `nee_current`；`vasopressor_burden` 为探索通道，默认关闭。
...
- `label_adjudications` 中 `adjudication_status = 'adjudicated'` 者以 `label_final_status` 覆盖（覆盖比例进 QA）；preliminary 与 final 双列保留。
...
| # | 事项 | 影响 | 处置 |
|---|---|---|---|
| PP-1 | D0 未锁定 → `time_origin` 主值不定 | 全部包的原点列 | 双口径并行产物（独立 run_id）；`training_ready = false` 直至锁定 |
