...
- 状态：**可执行版**。数据管线已正式冻结（31/31 项关闭），MIMIC 侧全部模型输入包 `training_ready=true`；eICU 包为外验评估包（Robustness under phenotype shift，`feasibility_only`）。
...
| 批大小 | 512 |
|  epochs | ≤100，early stop（validation iAUROC，patience=10） |
| 损失 | BCE（患者等权 `weight` 列，方案锁定） |
| 精度 | FP32（模型小，无需 AMP） |
| 种子 | `seed_root=20260730` 派生（`train/20260730/grud`、`…/tpc`） |
...
| 批大小 | 64（ECG 主导显存） |
| epochs | ≤50，early stop（validation iAUROC，patience=8） |
| 损失 | BCE（患者等权） |
| 精度 | **AMP bf16** |
...
3. **CV-SOFA≥3 亚组交互**（`sofa_realtime_strict_24h_cv`，患者级首个有效 landmark 值固定分层）；
4. 动态预警指标：预警 episode PPV、患者级 PPV、死亡预警成功率、lead time（阈值仅用 validation 锁定）。
